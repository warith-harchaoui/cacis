...
\subsection{ImageNet with FastText Semantic Cost}
\label{sec:imagenet}
We complement the fraud-detection benchmark with a large-scale image-classification reproducer in which the ground cost encodes \emph{semantic} rather than financial regret. The setup is identical in spirit but the units of $\mC$ change: instead of dollars of chargeback, each off-diagonal $C_{ij}$ measures how far two ImageNet class names lie in a frozen FastText embedding space (Equation~\ref{eq:semantic-cost}).

\paragraph{Protocol.} We train a torchvision \texttt{resnet50} from random initialization on ImageNet-1k. The cost matrix $\mC^{\mathrm{sem}} \in \R^{1000 \times 1000}$ is computed once from the WordNet class labels using the FastText Common Crawl English model ($d{=}300$); compound labels are tokenized and per-token vectors averaged, with subword fallback for out-of-vocabulary tokens. The artifact is mounted into the training container as a model file. Training uses a standard ImageNet recipe (90 epochs, SGD with Nesterov momentum, cosine LR with linear warm-up, mixed precision) driven by \texttt{torchrun} over $8\times$NVIDIA~A100 GPUs.

\paragraph{Metrics.} In addition to Top-1 and Top-5 accuracy, we report the realized \emph{semantic regret}
\[
...
\paragraph{Expected behavior.} We expect \CACIS{}-trained ResNet to match cross-entropy on Top-1 while improving $R_{\mathrm{sem}}$ --- i.e., where it misclassifies, it gravitates toward semantically adjacent classes. We expect the largest gains on classes whose nearest semantic neighbors are also visually confusable (the large \emph{dog} and \emph{bird} subtrees of ImageNet). A regularization-path schedule on $\varepsilon$ (\S\ref{sec:cacis}) is particularly useful here because the cost matrix is dense and the off-diagonal scale is much larger than in the fraud setting.

\paragraph{Reproducibility.} A self-contained subpackage (\texttt{examples/imagenet/}) ships with the supplementary code: cost-matrix precomputation, DDP-aware ImageNet loaders, ResNet factory, and the training loop. A Dockerfile produces a runnable image for AWS and GCP GPU instances. The complete benchmark numbers will be reported in the de-anonymized version.
